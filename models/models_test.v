...
fn test_format_large_units() {
	...
	assert format_large(123.45) == '123'
	assert format_large(4000.0) == '4.0K'
	assert format_large(3000000.0) == '3.0M'
	assert format_large(2500000000.0) == '2.5B'
	assert format_large(1500000000000.0) == '1.5T'
}

// 整值边界：旧实现会因 split('.')[1] 越界 panic
fn test_format_large_exact_boundaries() {
	assert format_large(1e12) == '1.0T'
	assert format_large(1e9) == '1.0B'
	assert format_large(1e6) == '1.0M'
	assert format_large(1e3) == '1.0K'
}

fn test_data_source_roundtrip() {
	for d in [DataSource.worldbank, .imf, .market] {
...
fn test_all_categories_complete_and_unique() {
	...
	for c in cats {
		...
		assert c.icon != ''
		assert c.source in ['worldbank', 'imf', 'market']
		assert !seen[c.id]
		seen[c.id] = true
	}
	// 新增的汇率 / 大宗商品分类必须存在
